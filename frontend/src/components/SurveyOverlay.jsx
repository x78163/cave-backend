 ...
 */
export default function SurveyOverlay({ map, renderData, anchorLat, anchorLon, heading, converter: externalConverter = null, muted = false }) {
  ...
  useEffect(() => {
    ...
    // Color scheme — purple for nearby cave overlays, normal amber/cyan for primary
    const wallColor   = muted ? '#a78bfa' : '#ffa726'
    const wallOpacity = muted ? 0.6 : 0.5
    const fillOpacity = muted ? 0.15 : 0.18
    const fillLower   = muted ? 0.08 : 0.10
    const lineColor   = muted ? '#c4b5fd' : 'var(--cyber-cyan, #00ffff)'
    const lineOpacity = muted ? 0.8 : 0.9
    const stationColor = muted ? '#c4b5fd' : '#00ffff'
    const symbolColor  = muted ? '#a78bfa' : '#ffa726'
    ...
    if (outlines.length > 0) {
      for (const o of outlines) {
        ...
        const wallStyle = {
          color: wallColor, weight: 1, opacity: wallOpacity,
        ...
        L.polygon(latlngs, {
          ...
          fillColor: wallColor,
          fillOpacity: isLower ? fillLower : fillOpacity,
        }).addTo(group)
    ...
    // Draw station markers + labels (skip when muted to reduce clutter)
    if (!muted && renderData.stations?.length > 0) {
      ...
      renderData.stations.forEach((station, idx) => {
        ...
        L.circleMarker(ll, {
          radius: isLower ? 2 : 4,
          color: stationColor,
          fillColor: isLower ? 'transparent' : stationColor,
    ...
    // Symbol icons at shot midpoints (skip when muted)
    if (!muted) {
      for (const ann of (renderData.shot_annotations || [])) {
        ...
        keys.forEach((key, i) => {
          ...
          if (!svg) return
          const html = colorize(svg, symbolColor)
          const size = 22
  ...
  }, [map, renderData, anchorLat, anchorLon, heading, muted])
